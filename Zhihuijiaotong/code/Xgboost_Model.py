...
from sklearn.model_selection import ParameterGrid  # 每个参数的网格具有不连续的数值，可用于使用python内置的函数遍历参数值组合
...
def xgboost_submit(df, params):
    ...
    train_df =train_df.dropna()   # 此处要修改调试
    ...
    regressor.fit(X_train, y_train, verbose=True, early_stopping_rounds=10, eval_metric=mape_in, eval_set=eval_set)
    # 输出特征重要度
...
create_feture('data/pre_training.txt', 'data/com_training.txt', lagging=lagging)
