...
def imputation_with_spline(file, to_file):
    df = pd.read_csv(file, delimiter=';', parse_dates=['time_travel_begin'], dtype={'link_ID':object})
    ...
    def date_trend(group):
        ...
        def nan_helper(y):
            return np.isnan(y), lambda z: z.nonezero()[0]
        ...
        if group.link_ID.values[0] in ['3377906282328510514', '3377906283328510514', '4377906280784800514',
            ...
            regr.fit(x(~nans).reshape(-1, 1), y[~nans].reshape(-1, 1))  # 变成一列
    ...
    df = pd.get_dummies(df, columns=['link_num', 'width', 'minute', 'hour', 'week_day', 'day', 'month'])
    print(df.head(20))

    feature = df.columns.values.tolist()
    train_feature = [x for x in feature if
                    x not in ['link_ID', 'time_interval_begin', 'travel_time', 'date', 'travel_time2', 'minute_trend',
                              'treval_time_std', 'date_trend']]
    train_df = df.loc[~df['travel_time'].isnull()]
    test_df = df.loc[df['travel_time'].isnull()].copy()
    print(train_feature)
    X = train_df[train_feature].values
    ...
    df = pd.merge(df, test_df[['link_ID', 'time_interval_begin', 'prediction']], on=['link_ID', 'time_interval_begin'], how='left')
...
def create_feture(file, to_file, lagging=5):
    ...
    df2 = pd.get_dummies(df2, columns=['week_hour', 'link_num', 'width'])
