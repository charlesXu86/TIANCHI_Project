...
@author: CharlesXu
@license: Q_S_Y_Q 
@file: Related_lagging.py
@time: 2018/3/1 9:43
@desc: lagging 时间特征   调参手段
"""
...
if __name__ == '__main__':
    df = pd.read_csv('data\\training.txt', delimiter=';', parse_dates=['time_interval_begin'], dtype={'link_ID':object})
    link_tops = pd.read_csv('E:\dataset\data\gy_contest_link_top.txt', delimiter=';', dtype={'link_ID':object})
    link_tops.fillna(' ', inplace=True)
    in_links_df = pd.DataFrame(link_tops['in_links'].str.split('#').tolist(), columns=['in_link1', 'in_link2', 'in_link3', 'in_link4'])
    out_links_df = pd.DataFrame(link_tops['out_links'].str.split('#').tolist(), columns=['out_link1', 'out_link2', 'out_link3', 'out_link4'])
    ...
    def applyParallel(dfGrouped, func):
        retLst = Parallel(n_jobs=multiprocessing.cpu_count())(delayed(func, check_pickle=False)(group) for name, group in dfGrouped)
        return pd.concat(retLst)

    def related_lagging(group):
        tmp = group[['link_ID', 'lagging']].copy().set_index('link_ID')
